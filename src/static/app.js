  ...
  async function fetchActivities() {
    try {
      ...
      activitySelect.innerHTML =
        '<option value="">-- Select an activity --</option>'; // Reset dropdown
                  ...
                  .map(
                    (email) => `
                      <li style="display: flex; align-items: center; gap: 0.5em;">
                        <span>${email}</span>
                        <span class="delete-participant" title="Remove participant" data-activity="${name}" data-email="${email}" style="cursor:pointer;color:#c00;font-weight:bold;">&#10006;</span>
                      </li>
                    `,
                  )
                  .join("")}
